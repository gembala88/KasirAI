...
const { verifyAccessToken: verifyAccessTokenForTest } = await import('../src/modules/auth/infrastructure/jwt.js');
...
describe('login', () => {
  ...
  it('rejects a disabled ERPNext user even with a hermes_role set', async () => {
    ...
    await expect(login('disabled@hermes.local', 'correct')).rejects.toMatchObject({ statusCode: 401 });
  });

  it('issues a real token pair for a valid, provisioned user', async () => {
    fetchMock.mockResolvedValue(jsonResponse(200, { message: 'Logged In', full_name: 'Budi Owner' }));
    erpNextClientMock.get.mockResolvedValue({ enabled: 1, hermes_role: 'Owner' });

    const result = await login('owner@hermes.local', 'correct');

    expect(result.user).toEqual({ email: 'owner@hermes.local', fullName: 'Budi Owner', role: 'Owner' });
...
describe('refreshAccessToken', () => {
  it('issues a new access token from a valid refresh token', () => {
    const { refreshToken } = issueTokenPair({ email: 'cashier@hermes.local', fullName: 'Cashier', role: 'Cashier' });
  ...
  it('rejects an access token presented as a refresh token (type confusion)', () => {
    const { accessToken } = issueTokenPair({ email: 'x@hermes.local', fullName: 'X', role: 'Owner' });
...
describe('requireRole', () => {
  ...
  it('rejects a request with no authenticated user at all', async () => {
    await expect(requireRole('Owner')(fakeRequest(undefined))).rejects.toMatchObject({ statusCode: 403 });
  });
});
