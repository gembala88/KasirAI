    ...
    VitePWA({
      ...
      manifest: {
        ...
        icons: [
          ...
          // PNG copies alongside the SVGs above — packaging tools that turn
          // this manifest into an Android/Windows app (PWABuilder, see
          // docs/PACKAGING.md) need a real raster icon; most don't accept
          // SVG. The 512px one is also declared "maskable" so Android can
          // safely crop it to a circle/squircle without losing the logo —
          // the source SVG already fills its full square canvas, so no
          // extra safe-zone padding was needed to make that work.
