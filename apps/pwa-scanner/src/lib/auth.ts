// Token storage + login (spec §1.4 NFR "Security": JWT-based auth).
// localStorage so a reload doesn't force re-login; cleared on logout or
// when a refresh attempt itself fails. Mirrors apps/dashboard/src/lib/auth.ts
// — same pattern, separate storage key so a browser with both apps open
...
const STORAGE_KEY = 'hermes-pwa-scanner-auth';
